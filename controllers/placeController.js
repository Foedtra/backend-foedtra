const { dbPlaces } = require("../configs/config");

// Add Place Cont
const addPlace = async (req, res) => {
  const { placeName, placeImage, placeDesc, placeMenu, contact, lat, lng } =
    req.body;

  const uid = res.locals.uid;
  // example image marker
  const icon =
    "https://firebasestorage.googleapis.com/v0/b/foedtra-app.appspot.com/o/Location.png?alt=media&token=8ba1d63e-a698-4f39-87cf-4c8b7bd1c01a";
...
const getAllPlace = async (req, res) => {
  ...
  if (name) {
    ...
    let filterName = places.filter((place) =>

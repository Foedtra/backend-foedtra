  ...
  await dbFoods.get().then((querySnapshot) => {
    ...
    if (name) {
      ...
      let filterName = foods
          .filter((food) => food.name.toLowerCase().includes(lowerName));
    ...
    if (from) {
      ...
      let filterFrom = foods
          .filter((food) => food.from.toLowerCase().includes(lowerFrom));
...
const getDetailFoodParams= async (req, res) => {
...
// getting food detail using request body
// const getDetailFoodBody = async (req, res) => {
//   const {foodId} = req.body;
//   const food = [];
//   await db.collection('foods').doc(foodId).get().then((doc) => {
//     if (doc.exists) {
//       food.push({
//         id: doc.id,
//         ...doc.data(),
...
const getFoodPredict = async (req, res) => {
  ...
  const {lat} = req.body;
  const {lng} = req.body;
  const apiKey = process.env.API_KEY_MAPS; // cannot load apiKey from env
  //   console.log(image);
  const apiPredict = {
    method: 'post',
    url: 'https://foedtra.as.r.appspot.com/predict', // our ML API
    data: {
      image: image,
    },
  };
